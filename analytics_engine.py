...
def compute_analytics(df: pd.DataFrame, user_df: pd.DataFrame) -> dict:
    ...
    # NEW metric for Opportunity Lens
    ...
    analytics['risk_radar'] = task_summary.sort_values(by='Risk Index', ascending=False) # Keep original name for compatibility
    analytics['risk_matrix'] = task_summary # Keep original name for compatibility

    # Opportunity Lens: Identify over-strengths (NEW)
    analytics['opportunity_lens'] = task_summary[task_summary['Avg_Score'] >= 0.75].sort_values('Competency_Score', ascending=False)

    # Talent pipeline: medium performers in critical tasks (Original)
    ...
    # Hidden stars and adjusted ranking (Original)
